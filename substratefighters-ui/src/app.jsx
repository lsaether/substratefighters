...
export class App extends ReactiveComponent {
	constructor() {
		...
		window.substrateFightersVersion = 2;
		addCodecTransform('FighterV' + window.substrateFightersVersion + '<Hash,Balance>', {
			id: 'Hash',
			dna: 'Hash',
			price: 'Balance',
			strength: 'u64',
...
class SubstratefightersSegment extends React.Component {
	constructor() {
		super()

		this.skAccount = new Bond
		this.skFighter1 = new Bond
		this.skFighter2 = new Bond
	}
	...
	render() {
		return <Segment style={{ margin: '1em' }} padded>
			<Header as='h2'>
				...
				<Header.Content>
					Substrate Fighters
				<Header.Subheader>There are <Pretty value={runtime.substratefighters.allFightersCount} /> fighters ready!</Header.Subheader>
				</Header.Content>
			</Header>
			<div style={{ paddingBottom: '1em' }}></div>
			<FighterCards count={runtime.substratefighters.allFightersCount} />
			<div style={{ paddingBottom: '1em' }}></div>
			<SignerBond bond={this.skAccount} />
			<TransactButton
				content="Create Fighter"
				icon='shield'
				tx={{
					sender: runtime.indices.tryIndex(this.skAccount),
					call: calls.substratefighters.createFighter()
				}}
			/>
			<div style={{ paddingBottom: '1em' }}></div>
			<If condition={calls.substratefighters.fight} then={<div>
				<InputBond
					bond={this.skFighter1}
					placeholder='Fighter ID 1'
					validator={n => (n.substr(0, 2) == '0x' && n.length == 66) ? n : null}
					iconPosition='left'
					icon='shield'
				/>
				<InputBond
					bond={this.skFighter2}
					placeholder='Fighter ID 2'
					validator={n => (n.substr(0, 2) == '0x' && n.length == 66) ? n : null}
					iconPosition='left'
					icon='shield'
				/>
				<TransactButton
					...
					tx={{
						sender: runtime.indices.tryIndex(this.skAccount),
						call: calls.substratefighters.fight && calls.substratefighters.fight(this.skFighter1, this.skFighter2)
					}}
				/>
			</div>} />
		</Segment>
